...
import (
	...
	"github.com/tevfik/seriesly/libs/go-humanize"
	"github.com/tevfik/seriesly/libs/gojson"
	"github.com/tevfik/seriesly/libs/timelib"
	"github.com/tevfik/seriesly/libs/gouchstore"
...
func dumpDocs(args []string, w http.ResponseWriter, req *http.Request) {
	...
	var getJsonKeys = false
	var jsonKeyList []string
...
//		return
	...
	_key, err := strconv.Atoi(req.FormValue("keys"))
	if err == nil {
		_key = _key
		getJsonKeys = true
		jsonKeyList = append(jsonKeyList, "","")
	}
	...
	err = dbwalk(args[0], from, to, func(k string, v []byte) error {
		...
		if !getJsonKeys {
			...
			l, err := jsonpointer.ListPointers(v)
			if err != nil {
				emitError(400, w, "Error listing pointers: %v", err.Error())
				return err
			}
			...
			jsonKeyList = append(jsonKeyList, keys...)
			keys = nil
		}
		return err
	})
	if getJsonKeys {
		x := convertString2ByteArray(jsonKeyList[2:])
		_, err = output.Write(x)
	}
}

func convertString2ByteArray(s []string) []byte{
	var x = []byte{}
	x = append(x,'{')
	x = append(x,'"')
	x = append(x,'k')
